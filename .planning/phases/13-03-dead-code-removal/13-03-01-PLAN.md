...
must_haves:
  truths:
    - "All #[allow(dead_code)] markers are reviewed and acted on (except TODO futures)"
    ...
    - "FFI declarations in backend.rs are verified as actively used (not dead code)"
  ...
  artifacts:
    ...
    - path: "src/backend/hip_backend/backend.rs"
      ...
      min_lines: 1
      action: "FFI block marker retained with comment explaining active use"
  key_links:
...
<objective>
Resolve `#[allow(dead_code)]` markers, suppress naming convention warnings for GGUF compatibility, and verify FFI declarations are actively used.

Purpose: Clean up dead code suppressions, remove truly unused code, properly document exceptions for external format compatibility, and confirm FFI bindings are not dead code. This reduces a significant category of compiler warnings while preserving code that is either actively used or matches external specifications.

Output: 5 fewer `#[allow(dead_code)]` markers (with FFI block verified and documented), truly unused functions removed, GGUF naming warnings properly suppressed, FFI usage verified.
</objective>
...
<task type="auto">
  <name>Task 6: Verify FFI declarations are actively used (CLEAN-02)</name>
  <files>src/backend/hip_backend/backend.rs</files>
  <action>
    Verify that all FFI declarations in src/backend/hip_backend/backend.rs are actively used and document the finding.

    Research (13-03-RESEARCH.md) confirmed all FFI functions are used:
    - hipInit, hipGetDeviceCount, hipSetDevice - device management
    - hipMalloc, hipFree - memory management
    - hipMemcpy* - data transfer
    - hipStream*, hipEvent* - async operations and timing
    - hipModule* - kernel loading
    - hiprtc* - runtime compilation

    Action: Add a comment above the `#[allow(dead_code)]` marker on line 11 explaining this:
    ...
    Do NOT remove any FFI declarations. Do NOT remove the #[allow(dead_code)] marker.

    Commit message: "docs(13-03-01): document FFI declarations are actively used, not dead code"
  </action>
  <verify>grep -A2 "extern \"C\"" src/backend/hip_backend/backend.rs | head -5 (should show explanatory comment)</verify>
  <done>FFI block documented as actively used, not dead code (CLEAN-02 satisfied)</done>
</task>
...
2. grep -rn '#\[allow(dead_code)\]' src/ shows 3 markers (backend.rs:11 FFI block with comment, backend_registry.rs:302,367 TODO fields)
3. Q[2-6]_K naming warnings are suppressed
4. FFI block has explanatory comment above #[allow(dead_code)]
5. Full test suite still passes: cargo test --all-targets
</verification>

<success_criteria>
- 5 fewer #[allow(dead_code)] markers (from 8 to 3, FFI block retained with documentation)
- Q[2-6]_K naming warnings suppressed
- Truly unused functions removed
- FFI usage verified and documented
