...
<task type="auto">
  <name>Task 1: Add packed_simd dependency</name>
  <files>Cargo.toml</files>
  <action>
    Add `packed_simd` to dependencies in Cargo.toml:

    ```toml
    [dependencies]
    packed_simd = { version = "0.3.8", optional = false }
    ```

    Also add to main.rs if feature detection is needed:
    ```rust
    #[cfg(feature = "cpu-simd")]
    use packed_simd::{f32x4, i32x4, m32x8};
    ```
  </action>
  <verify>cargo check passes</verify>
  <done>packed_simd available in project, cargo check passes</done>
</task>
            ...
            for i in (0..m).step_by(TILE_SIZE) {
                for j in (0..n).step_by(TILE_SIZE) {
                    ...
                    for kk in (0..k).step_by(TILE_SIZE) {
                        let a_vec = f32x4::from_slice_unaligned(&a[i * k + kk..]);
                        let b_vec = f32x4::from_slice_unaligned(&b[kk * n + j..]);

                        sum += a_vec * b_vec;
                    }

                    // Store result
                    let result = sum.toArray();
